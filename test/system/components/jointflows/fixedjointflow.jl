...
@testset "FixedJointFlow" begin
    ...
    let m = makeconvdata() # model data for converter
        ...
        @test all(mass(input(portstructure(c))["ff"])[i] == Float64(i) for i in eachstep(sim(mc)))
    ...
    let m = makeconvdata() # model data for converter
        ...
        @test all(mass(input(portstructure(c))["ff"])[i] == Stepwise(Hourly(Float64.(1:5), sim(c).mesh))[i] for i in eachstep(sim(mc)))
    ...
    let m = makeconvdata() # model data for converter
        ...
        @test all(mass(input(portstructure(c))["ff"])[i] == 5. for i in eachstep(sim(mc)))
