...
function _mk_ports(::Type{T}, sim::Sim, mesh::TimeMesh, mname::String, carrier) where {T}
    ps  = PortStructure{T}(sim)
    f_ft = Stepwise(sim, mesh; basename=mname*"_ft", lb=0.0)
    f_tf = Stepwise(sim, mesh; basename=mname*"_tf", lb=0.0)
...
_build(m::ACLine, mname::String) = ACLineModel{exptype(m.sim)}(m, _mk_ports(exptype(m.sim), m.sim, m.mesh, mname, m.from))

# DC line: same port layout; no KVL
_build(m::DCLine, mname::String) = DCLineModel{exptype(m.sim)}(m, _mk_ports(exptype(m.sim), m.sim, m.mesh, mname, m.from))
