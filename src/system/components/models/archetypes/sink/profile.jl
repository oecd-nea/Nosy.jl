...
"""
function ProfileSink(carrier::AbstractCarrier, profile; cutoff::Number=Inf64)
    ...
    return ProfileSink(s, carrier, Stepwise(_profile, s.mesh), Float64(cutoff))
end
...
function build(m::ProfileSink, cname::String)
    ...
    addinput!(ps, "input", cname, Port(m.carrier, vin))
